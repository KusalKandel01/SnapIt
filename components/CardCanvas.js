  ...
  if (layout === 'quote') {
    return (
      <div ref={ref} style={{ ...wrapStyle, background: 'linear-gradient(160deg, var(--ink) 0%, #1b1c1f 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '50px 34px', textAlign: 'center' }}>
        ...
        <div style={{ fontWeight: 700, fontSize: 13, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--gold)' }}>&mdash; {quoteAuthor}</div>
        {cornerTag && <div style={{ position: 'absolute', left: 0, right: 0, bottom: 16, fontFamily: font, fontSize: 10.5, letterSpacing: '.1em', color: 'var(--gold)', textTransform: 'uppercase' }}>{cornerTag}</div>}
      </div>
    );
  }
