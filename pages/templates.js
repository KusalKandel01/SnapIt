    ...
    <Layout>
      ...
      {GROUPS.map(group => (
        <div key={group.label} style={{ marginBottom: 30 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 16, letterSpacing: '.04em', color: 'var(--brass)', marginBottom: 14 }}>{group.label}</h2>
            ...
            {group.items.map(item => {
              ...
              return (
                <div key={item.name} className="card-panel" style={{ cursor: 'pointer', padding: 12 }} onClick={() => usePreset(item)}>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 13, margin: '4px 0 2px 0' }}>{item.name}</h3>
                  <p style={{ color: 'var(--rule-light)', fontSize: 11 }}>Click to use</p>
                </div>
              );
            })}
          </div>
